...
- **One Vercel serverless function**, `api/lead.js`, which receives the "Request Investor Access" form as JSON, validates it, drops honeypot spam, and emails the lead to `invest@vitabahn.com` via SMTP using `nodemailer` (the project's only runtime dependency).
...
**Where personal data enters/leaves:** personal data (first name, last name, business email, organization, optional ticket range, optional free-text message, plus a consent flag) is entered in the form, POSTed to the Vercel function, and emailed onward to `invest@vitabahn.com` via Porkbun SMTP. No database, cookie, `localStorage`, `sessionStorage`, or analytics is involved anywhere.
...
**Evidence.** The footer (`index.html:747-751`) contains only "Brief"/"Contact" anchor columns, a `mailto:invest@vitabahn.com`, a confidentiality disclaimer, and the copyright line. A repo grep for `Impressum`/`GmbH`/`UG`/`Handelsregister`/`USt` returns no legal-entity or address information anywhere.
...
`api/lead.js:18` defaults `LEAD_TO = 'invest@vitabahn.com'`, and the config-presence guard (`api/lead.js:73-76`) validates `SMTP_USER`/`SMTP_PASS` but not the recipient wiring. A fork/redeploy that forgets to set `LEAD_TO` would silently email the baked-in VitaBahn address rather than failing closed. A misconfiguration footgun, not attacker-exploitable. **Fix (optional):** require `LEAD_TO` explicitly and fail closed if unset.
