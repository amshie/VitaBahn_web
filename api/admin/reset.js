...
// (investors, access requests, documents, invites, access logs, signed NDA
// submissions). Admin accounts are preserved so the console stays accessible.
// Level-0 only, Origin-checked, and requires the explicit confirm phrase. The
// wipe itself is written to the (now otherwise empty) audit log.
...
export default async function handler(req, res) {
  ...
  await logEvent({
    ...
    detail: `database reset — cleared ${cleared.investors} investors, ${cleared.requests} requests, ${cleared.documents} documents, ${cleared.invites} invites, ${cleared.logs} logs, ${cleared.ndaSubmissions} NDA submissions (admins preserved)`,
